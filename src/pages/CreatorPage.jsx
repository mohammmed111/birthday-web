...
export default function CreatorPage() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{ background: '#051937' }}>
      {/* Noise texture */}
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden" aria-hidden="true">
        <div
          ...
          style={{ background: 'radial-gradient(circle, #E8B54D, transparent)' }}
        />
        <div
          className="absolute -bottom-40 -start-40 w-96 h-96 rounded-full opacity-15 blur-3xl"
          style={{ background: 'radial-gradient(circle, #0F52BA, transparent)' }}
        />
        <div
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <header className="pt-12 pb-2 px-6 text-center">
          ...
          >
            ...
            <h1 className="font-headline text-4xl md:text-5xl lg:text-6xl text-tertiary leading-tight">
              أنشئ لحظة{' '}
              <span className="text-transparent bg-clip-text"
                style={{ backgroundImage: 'linear-gradient(135deg, #E8B54D, #F9F0DB, #E8B54D)' }}>
                لا تُنسى
              </span>
            </h1>
            <p className="mt-4 text-tertiary/60 font-body text-base md:text-lg max-w-md mx-auto leading-relaxed">
          ...
          {['اكتب الرسالة', 'احصل على الرابط', 'شارك المفاجأة'].map((step, i) => (
            <React.Fragment key={step}>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-tertiary/50 text-xs font-label hidden sm:block">{step}</span>
              </div>
        ...
        <main className="flex-1 flex items-start justify-center px-4 py-8">
          ...
          >
            <div className="gradient-border rounded-3xl p-6 md:p-8 bg-primary-950/70 backdrop-blur-xl shadow-2xl shadow-neutral-950/60">
              <AnimatePresence mode="wait">
                {!generatedLink ? (
                  ...
                  >
                    <CreateForm onLinkCreated={setGeneratedLink} />
                  </motion.div>
                ) : (
                  <motion.div
                    key="result"
